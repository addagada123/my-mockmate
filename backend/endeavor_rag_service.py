...
def _deduplicate_questions_consensus(
    ...
    def _quality_score(q: Dict) -> float:
        ...
        score += float(min(30.0, len(q_text.split()) * 2.0))
        score += float(min(40.0, len(a_text.split()) * 1.5))
...
def build_resume_only_questions(
    ...
    for lang in coding_langs:
        if lang in skill_lc:
            ...
            break
    # Ensure domain is a string for startswith
    domain_str: str = domain
    if not domain_str.startswith("Mechanical"):
        primary_lang = "Python"
    else:
        primary_lang = "MATLAB"
...
def analyze_resume_focus(resume_text: str, skills: List[str]) -> Dict[str, Any]:
